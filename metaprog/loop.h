...
template<class I, class N, class M> struct loop_iterator_impl
{
   ...
   using prev  = loop_iterator_impl<libmda::metaprog::prev<I>, N, M>;
   using next  = loop_iterator_impl<libmda::metaprog::next<I>, N, M>;
   using deref = I;
};
template<class N, class M> struct loop_iterator_impl<N,N,M>
{
   using tag   = bidirectional_iterator_tag;
   using next  = loop_iterator_impl<libmda::metaprog::next<N>, N, M>;
   using deref = N;
};
template<class N, class M> struct loop_iterator_impl<M,N,M>
{
   using tag   = bidirectional_iterator_tag;
   using prev  = loop_iterator_impl<libmda::metaprog::prev<M>, N, M>;
   using deref = void;
};
