...
void for_each_in_arg_pack_subset(F&& f, metaprog::index_list<Is...> const& i, Ts&&... args)
{
...
detail::invoker<detail::indexed_type<Is, metaprog::Nth_type_of<Is, Ts...> >...> invoker(
              f,
              (utility::nth_value_of<Is>(std::forward<Ts>(args)...))...
              );
}
...
{
   for_each_in_arg_pack_subset(f, metaprog::index_range<0, sizeof...(Ts)>(), std::forward<Ts>(args)...);
}
